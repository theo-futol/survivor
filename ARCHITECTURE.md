...
**Authentication and authorization.** Login issues a JWT (`jose`, HS256) stored in the `cartepro_token` cookie. `CartePro/proxy.ts` gates page routes per role and redirects to the role's home (`/` employee, `/employer` company, `/partner` partner, `/admin` admin). API routes are gated by `lib/roles-config.ts`, a single map from `METHOD /path` to the allowed roles — extend it whenever a protected handler is added. Roles are `ADMIN`, `COMPANY`, `PARTNER`, `EMPLOYEE`.
