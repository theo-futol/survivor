 ...
 *       propre entreprise ; un `ADMIN` peut modifier n'importe laquelle. Le champ `isPartner` est
 *       piloté par le serveur et ne peut jamais être fourni. Les champs `verified`, `active`,
 *       `agentId`, `reasonId` et `kbisId` relèvent de la validation administrative : seul un
 *       `ADMIN` peut les fournir, sous peine de 403. Le passage de `verified` à `true` envoie un
 *       email de validation à l'entreprise et fait passer à `ACCEPTED` le compte utilisateur créé
 *       avec elle à l'inscription, qui peut alors se connecter ; repasser `verified` à `false` le
 *       remet à `PENDING`. `active` suspend ou réactive le compte : un employeur suspendu
 *       (`active = false`) disparaît des listings et son compte ne peut plus se connecter, sans
 *       perdre son historique ni celui de ses salariés ; le repasser à `true` le restaure
 *       intégralement. Les comptes salariés ne sont pas touchés par ce champ : ils relèvent de
 *       leur propre statut `active`.
 *     security:
 *       - bearerAuth: []
 *       - cookieAuth: []
 *     parameters:
