...
CartePro est un outil permettant aux employeurs de créditer leurs salariés de crédits *(simulation)* utilisables auprès d'un réseau de partenaires référencés. Le dispositif s'inscrit dans une démarche de valorisation du travail et de soutien à l'économie locale.
...
8. **Validation de la transaction** — Le partenaire scanne le QR code (ou le saisit manuellement) pour valider la transaction.
9. **Suivi post-transaction** — Le salarié consulte son historique ; le partenaire consulte son tableau de bord financier (montants reçus *(simulation)*, transactions par période).
10. **Pilotage national** — L'administration suit le tableau de bord national (volume de transactions, partenaires actifs, répartition géographique) et gère les comptes (activation, suspension, clôture).

Une carte géographique est mise à disposition des salariés pour faciliter la recherche de partenaires référencés grâce à la géolocalisation de l'utilisateur (optionnelle).

## Droits et obligations des utilisateurs et de l'éditeur

Quatre types d'accès existent : salarié, employeur, partenaire et administration.
...
**L'administration** peut valider les inscriptions des partenaires et des salariés et suivre le tableau de bord national. Elle doit gérer les comptes conformément aux règles établies et garantir la sécurité des données. Elle peut notamment :

- bannir un compte en cas de non-respect des conditions d'utilisation ou de comportement frauduleux ;
- accepter ou refuser une inscription partenaire / employeur selon les critères définis.

## Conditions d'utilisation

L'inscription est obligatoire pour les partenaires, les employeurs et les salariés afin d'accéder aux services proposés ; elle vaut acceptation des présentes CGU.
...
**Solde insuffisant** *(simulation)* — La transaction est refusée par le partenaire, avec notification au salarié, qui est invité à recharger son compte.

**Solde non consommé en fin de période** *(simulation)* — Le solde restant au 31 décembre reste utilisable jusqu'au dernier jour de février de l'année suivante. Passé cette date, il est reporté automatiquement pour un an (une seule fois) ou peut être converti en don à une association.

**Annulation d'une transaction validée** — L'administration ne peut en aucun cas annuler une transaction. Les transactions sont immuables. Un remboursement peut être effectué dans certains cas.

**Litige entre salarié et partenaire** — En cas de désaccord sur une transaction, les parties sont invitées à résoudre le litige à l'amiable. L'administration n'intervient pas dans les litiges commerciaux entre salariés et partenaires.
